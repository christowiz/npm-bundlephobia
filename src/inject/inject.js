	...
	const readyStateCheckInterval = setInterval(async function() {
		if (document.readyState === 'complete') {
			...
			for (let i = h3Tags.length - 1; i >= 0; --i) {
				var tag = h3Tags[i];
				if (tag.textContent && tag.textContent === 'last publish') {
					siblingEl = tag;
					break;
				}
			}
			...
			if (options.fetchOnPageLoad) {
				fetchBundleSizes(packageName).then(handleFetchResponse).catch(handleFetchError);
			}

			function handleFetchResponse(data) {
				paragraph.innerHTML = `Size: <span>${formatBytes(data.size)}</span> | Gzip: <span>${formatBytes(
					data.gzip
				)}</span> <a class="external-link" href="https://bundlephobia.com/result?p=${packageName}" target="_blank"><img src="${externalLink()}"></a>`;
